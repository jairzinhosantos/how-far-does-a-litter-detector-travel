...
- **MSc in Artificial Intelligence (UNI)**

| Resource | Where |
|---|---|
| Paper (IEEE 2-col, EN) | `reports/paper/main.pdf` |
| Poster (A0 vertical, EN) | `reports/poster/poster.pdf`|
...
| 7 | `06_evaluacion_consolidada.ipynb` - final tables, figures, TIDE | ~2 min |

**Training-data shortcut (Drive).** To skip steps 1--4 entirely, the project's
[Google Drive folder](https://drive.google.com/drive/folders/1MeKB5Bl212Jx-aE9R-X_dK1VaEyEiO4b?usp=sharing) carries `training_bundles/` with `gold_colab_p0.zip`
(4.5 GB, Faster R-CNN/FCN/VGG) and `gold_colab_p1.zip` (13 GB, YOLO): unzip them
into `data/gold/` to train locally, or upload them to your own MyDrive and run
notebooks 04/05 on Colab - they unpack them automatically. The bundles carry the
train/val data; evaluation ground truths come from the `gold_coco_gt.zip` asset.
The same folder mirrors the raw datasets and every Release asset.
...
If any link is unavailable, the [Google Drive folder](https://drive.google.com/drive/folders/1MeKB5Bl212Jx-aE9R-X_dK1VaEyEiO4b?usp=sharing) carries the same files
(release mirror, training bundles and raw datasets).
